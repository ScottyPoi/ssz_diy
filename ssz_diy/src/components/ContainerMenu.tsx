...
let basics = ["Boolean", "Uint8", "Uint16", "Uint32", "Uint64", "Uint128", "Uint256"]
    ...
    return (<div>
        ...
        <div className="row">
            <div className="col">
                <div className="row">
                    Basic Types
                    <SelectElementType types={basics} name="Select Basic Type" setType={(e: string) => setSelection(e)}/>
                </div>
                <div className="row">
                    Composite Types
                </div>
            ...
            <div className="col">
                <div className="row">
                    <button className="btn btn-warning" type='button' onClick={() => {addField(selection)}} >
                        Add {selection} Field to Container at index {fields.length}
                    </button>
                </div>
            </div>
            <div className="col">Container Description
            {fields.map((field, idx) => {
                return (
                    <div className="row">
                        {idx}: {field}  
                        <button className="btn" type='button' onClick={() => removeField(idx)}>X</button>
                        {idx !== 0 && <button className="btn" type='button' onClick={() => moveUp(idx)}>Up</button>} 
                        {idx !== fields.length - 1 && <button className="btn" type='button' onClick={() => moveDown(idx)}>Down</button>} 
